...
from imgclas_lo.utils import create_dir_tree
from imgclas_lo.data_utils import load_data_splits, compute_meanRGB, compute_classweights, load_class_names, load_location_names, data_sequence, data_sequence_lo, \
    json_friendly
...
tf.logging.set_verbosity(tf.logging.ERROR) #Todo lo que tenga el tag ERROR o más grave es sacado por pantalla

# Dynamically grow the memory used on the GPU (https://github.com/keras-team/keras/issues/4161)
gpu_options = tf.GPUOptions(allow_growth=True) #Preparo el entorno de gpu para la ejecución
tfconfig = tf.ConfigProto(gpu_options=gpu_options)
sess = tf.Session(config=tfconfig)
K.set_session(sess)

# Empiezo el método propiamente
def train_fn(TIMESTAMP, CONF):
    ...
    if CONF['training']['use_location']:
        X_train, y_train, locations_train = load_data_splits(splits_dir=paths.get_ts_splits_dir(),
        ...
        if (CONF['training']['use_validation']) and ('val.txt' in os.listdir(paths.get_ts_splits_dir())):
            X_val, y_val, locations_val = load_data_splits(splits_dir=paths.get_ts_splits_dir(),
                                            im_dir=paths.get_images_dir(),use_location = CONF['training']['use_location'],
                                            split_name='val')
        else:
    ...
    if not CONF['training']['use_location']:
    ...
    #Calculamos el número de localizaciones distintas que hay
    if CONF['training']['use_location']:
         CONF['model']['num_locations'] = len(location_names)
    ...
    if CONF['dataset']['mean_RGB'] is None:
        ...
        print('Mean_rgb:' + CONF['dataset']['mean_RGB'])
        print('std_RGB' + CONF['dataset']['std_RGB'])
    #Create data generator for train and val sets
    
    if not CONF['training']['use_location']:
        print("usando generator sin location")
    ...
    if CONF['training']['use_location']:
        print("usando generator CON location")
        train_gen = data_sequence_lo(X_train, locations_train, y_train,
                                  batch_size=CONF['training']['batch_size'],
                                  num_classes=CONF['model']['num_classes'],
                                  num_locations=CONF['model']['num_locations'],
                                  im_size=CONF['model']['image_size'],
                                  mean_RGB=CONF['dataset']['mean_RGB'],
                                  std_RGB=CONF['dataset']['std_RGB'],
                                  preprocess_mode=CONF['model']['preprocess_mode'],
        ...
        #for a,b in train_gen:
            #aqui
            #print(a[0][0].shape)
            #print(type(a[1][0]).shape)
            #print(len(b[0]))
          
        if CONF['training']['use_validation']:
            val_gen = data_sequence_lo(X_val, locations_val, y_val,
                                    batch_size=CONF['training']['batch_size'],
                                    num_classes=CONF['model']['num_classes'],
                                    num_locations=CONF['model']['num_locations'],
                                    im_size=CONF['model']['image_size'],
    ...
    if not CONF['training']['use_location']:
        ...
        # Pendiente de comprobar qué pasa con las layers de locations si selecciono el modo fast
        if CONF['training']['mode'] == 'fast':
            for layer in base_model.layers:
                layer.trainable = False
    
    if CONF['training']['use_location']:
        # Procedemos creando una red que tiene layers DENSAS de localizacion
        # Launch the training
        t0 = time.time()
        #print("hemos llegado aca ---------------------------")
        # Create the model and compile it
        model, base_model = model_utils.create_model_lo(CONF)
